...
public class CommitCostController extends BaseController{
    ...
    public ModelAndView home(ModelAndView modelAndView, HttpServletRequest request){
        List<commitCost> commitCostList= this.costCommitDao.findAll("select commitCost.id,commitCost.placeId,reportPlace.place,reportPlace.subPlace,commitCost.level,commitCost.count,commitCost.totalCost from commitCost commitCost,reportPlace reportPlace where commitCost.isDelete = 0 and commitCost.placeId = reportPlace.realId",request);
        ...
        for(int i=0;i<commitCostList.size();i++) {
            ...
            cost.setCount(Integer.parseInt(result.get(5).toString()));
            cost.setRemark(Double.parseDouble(result.get(6).toString()));
